      ...
      >
        <el-form label-width="120px" style="width: 100%">
          <el-row>
            <el-col :span="24" class="col_border">
              <el-form-item label="量化名称：" style="margin-bottom: 1px">{{
                detailData.name
              }}</el-form-item>
              <el-form-item
                v-if="!unpass"
              ...
              >
                <el-input
                  ...
                  label="证据描述  ："
                  class="resizeNone"
                  v-model="form.evidenceDescription"
                  placeholder="请输入内容"
                ></el-input>
              </el-form-item>
            </el-col>
        ...
        <div slot="footer" class="dialog-footer">
          <el-button
            ...
            style="margin-top: 10px; margin-left: 380px"
            @click="dialogVisible3 = false"
            >取消
          </el-button>
          <el-button
            icon="el-icon-scissors"
            :disabled="curStatus !== 0"
            type="info"
...
export default {
  ...
  methods: {
    deletepic(data) {
      ...
      for (var i = 0; i < this.attachs.length; i++) {
        ...
        if (i != data) {
          ...
          this.form.attach += this.attachs[i].substring(
            49,
            this.attachs[i].length
          );
          this.form.attach += ";";
        }
      }
    ...
    deepTree(treedata) {
      ...
      treedata.forEach((item) => {
        if (item.childNode.length === 0) {
          if (item.status === "不通过") _this.listData.unshift(item);
          else _this.listData.push(item);
          return;
        } else {
    ...
    async addEvidenceFile() {
      ...
      if (
        this.form.attachDescrption == "" ||
        this.form.evidenceDescription == ""
      ) {
    ...
    async updateScore(data) {
      ...
      if (data.status == "不通过") this.unpass = false;
            ...
            if (attach != null) {
              ...
              for (var i = 0, j = 0, k = 0; i < arr.length - 1; i++) {
                ...
                if (
                  ...
                  this.attachs[j] =
                    "http://39.98.173.131:7000/resources/QHSEEvidence/" +
                    arr[i];
                  j++;
                } else {
                  if (arr.length != 0)
    ...
    confirmSubmit() {
      if (this.tableID) {
        ...
        for (var i = 0; i < this.listData.length; i++) {
          if (
            this.listData[i].status == "未提供" ||
            this.listData[i].status == "不通过"
          )
            flag = 1;
        }
        if (flag == 1) this.$message.error("无法推送");
        if (flag == 0) {
