			...
			<el-form :inline="true" v-model="filterQuery" @submit.native.prevent="handleSearch">
				<el-form-item label="文件名称：">
					<el-autocomplete v-model="inputRegName" clearable :fetch-suggestions="querySearch" placeholder="请输入文件名称查询"
					 :trigger-on-focus="true" style="width:200px">
					</el-autocomplete>
				</el-form-item>
				<el-form-item label="发布机构：">
					<treeselect filterable :multiple="false" :options="options" placeholder="请选择发布机构" v-model="filterQuery.publishComCode"
					 style="width:200px" />
				</el-form-item>
				...
				<el-form-item label="执行时间：">
					<el-date-picker v-model="dateRange" type="daterange" align="right" range-separator="至" start-placeholder="开始日期"
			...
			handleGetFileList() {
				...
				GetFileList(this.filterQuery).then((res) => {
					...
					for (let i = 0; i < this.tableData.length; i++) { //获取模糊查询，文件名下拉框
						this.regNameListAyy.push({
							"value": this.tableData[i].regName
						})
					}
					this.loading = false
				}).catch((err) => {
