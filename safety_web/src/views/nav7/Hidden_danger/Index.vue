      ...
      <el-form
        ...
        <el-row>
            <el-form-item label="问题状态：">
              <el-select style="width:280px" v-model="filterQuery.status" placeholder="问题状态" clearable>
              <el-option v-for="item in statuses" :key="item.id" :label="item.name" :value="item.name"></el-option>
              </el-select>
            </el-form-item>
            ...
            <!-- <el-form-item label="问题重复：">
              <el-select  style="width:280px" v-model="filterQuery.problemRepeat" placeholder="问题重复性" clearable>
              <el-option v-for="item in repeats" :key="item.id" :label="item.name" :value="item.name"></el-option>
            </el-select>
          </el-form-item> -->
          &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; 
          <el-form-item>
              <el-button type="primary" native-type='submit' icon='el-icon-search' style="width:120px">查询</el-button>
            </el-form-item>
            <el-form-item>
              <el-button style="background-color:#3399CC;color:#FFFFFF;width:120px"  @click="export2Excel ()">导出Excel</el-button>
            </el-form-item>
        </el-row>
      </el-form>
       <el-table
    ...
    getDate(){
      ...
      this.excelData=GetProblemList1(this.filterQueryExcel).then((data) => {
        ...
        require.ensure([], () => {
          ...
          '问题级别','问题类别1','问题类别2','问题重复性','问题原因','检查人员','整改描述','是否延期',
          ...
          'problemClass2','problemRepeat','source','checkPersonName','rectiMeasure','isPostpone',
    ...
    gettaskName(){
      GetProblemList1(this.fitertask).then((res) => {  
        ...
        for(var i=0;i<this.taskAyy.length;i++){
          this.taskNameList.push({"value": this.taskAyy[i].taskName})
        }
        this.loading = false
      }).catch((err) => {
        this.$message.error(err.message)
      ...
      checkFilterParams () {
        ...
        if (this.dateRange && this.dateRange.length === 2) {
          this.filterQuery.checkDate1 = this.dateRange[0]
          this.filterQuery.checkDate2 = this.dateRange[1]
          } else {
