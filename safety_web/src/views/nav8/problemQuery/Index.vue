<template>
  <div>
    <div class="page-title">我录入的问题</div>
      ...
      <el-form v-if="this.client === 'android'"
        ...
        <el-row > 
            ...
            <el-form-item label="问题状态：">
              <el-select style="width:250px" v-model="filterQuery.status" placeholder="选择问题状态" clearable>
        ...
        <el-row>
            ...
            <el-form-item label="问题级别：">
              <el-select style="width:250px" v-model="filterQuery.problemRank" placeholder="问题级别" clearable>
  ...
  <el-table-column
    ...
    <el-table-column
      ...
      width="80">
       ...
       <el-table-column label="操作" align="center" header-align="center"   width="280px">
            <template slot-scope="scope">
            <div class="inline-td">
              <router-link :to='{name: "PersonalProblemsEdit1", params: {id: scope.row.problemID}}' v-if="scope.row.status === '未审核'&& scope.row.planID >=0">
                <el-button size="mini" type="primary" icon="el-icon-edit-outline">修改</el-button>
              </router-link>
            <router-link :to='{name: "PersonalProblemsEdit1", params: {id: scope.row.problemID}}' v-else-if="scope.row.status === '审核未通过'&& scope.row.planID >=0">
                <el-button size="mini" type="primary" icon="el-icon-edit-outline">修改</el-button>
              </router-link>
            <router-link :to='{name: "PersonalProblemsEdit2", params: {id: scope.row.problemID}}' v-else-if="scope.row.status === '未审核'&& scope.row.planID <0">
                <el-button size="mini" type="primary" icon="el-icon-edit-outline">修改</el-button>
              </router-link>
            <router-link :to='{name: "PersonalProblemsEdit2", params: {id: scope.row.problemID}}' v-else-if="scope.row.status === '审核未通过'&& scope.row.planID <0">
                <el-button size="mini" type="primary" icon="el-icon-edit-outline">修改</el-button>
              </router-link>
              <router-link :to='{name: "", params: {id: scope.row.problemID}}' v-else>
                <el-button size="mini"  type="primary" icon="el-icon-edit-outline" disabled>修改</el-button>
              </router-link>          
                &nbsp;&nbsp;
                <el-button  type='info' size="mini" icon='el-icon-view' @click="open(scope.row.modification)" v-if="scope.row.status ==='审核未通过'">审核意见</el-button>
                <el-button  type='info' size="mini" icon='el-icon-view' :disabled=true v-else>审核意见</el-button>
                &nbsp;
...
export default {
  ...
  methods:{
    ...
    // },
    getStatusDict () {
      GetDictionary({name: "问题总状态", type: "all"}).then((res) => {
    ...
    gettaskName(){
        GetPersonalProblemList(this.fitertask).then((res) => {
        console.log(JSON.stringify(res))  
        this.taskAyy = res.data.list
        // console.log(JSON.stringify(this.taskAyy))
