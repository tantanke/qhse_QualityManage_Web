      ...
      <div class="form-content">
        ...
        <el-form label-width="120px" :model="formData"  ref="formData" :rules="rules">
          ...
          <el-form-item label="检查级别：" prop="checkRank">
            <el-select  clearable v-model="formData.checkRank" placeholder="检查级别" style="width: 100%;">
              <el-option
                v-for="item in ranks"
                :key="item.id"
                :label="item.name"
          ...
          <el-form-item label="检查类型：" prop="checkType" >
            <el-select  clearable v-model="formData.checkType" placeholder="检查类型" style="width: 100%;"  filterable>
              <el-option
          ...
          <el-form-item label="文档附件：">
            <el-upload
              ...
              <div slot="tip" class="el-upload__tip">请上传一个word文档或者pdf文件，且不超过12M。</div>
            </el-upload>
          </el-form-item>
          <el-form-item>
  ...
  data() {
    return {
      ...
      rules:{
        ...
        checkRank :[{required: true, message: '请选择检查级别', trigger: 'blur' }],
  ...
  methods: {
    handleSubmit (formName) {
      this.$refs[formName].validate((valid) => {
          if (valid) {  
            // let a= this.formData.leaderIDs
            // this.formData.leaderIDs=a.toString() 
            this.formData.leaderIDs=this.formData.leaderIDs.join(',')
            this.formData.inputType = 'task_management'
            // alert(this.formData.inputType)
    ...
    handleGetDate(){
        ...
        this.formData.issueDate = year + seperator + month + seperator + strDate;
        // alert(this.formData.issueDate)

    },

    // handleLeaderSearch (val='') {
    //   this.leaderLoading = true
    //   GetEmployees({name: val,inputType :'task_management',type: "all"}).then((res) => {
    ...
    handleLeaderSearch (val='') {
      this.leaderLoading = true
      GetEmployees({name: val, type: "all",inputType:"task_management"}).then((res) => {
