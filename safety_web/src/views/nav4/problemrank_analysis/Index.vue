<template>
  <div>
    <div class="page-title" style="width:100%">按问题级别分析</div>
...
import {GetProblemRankAnalysis} from './problemRankAnalysis.js'
...
//加载条件
    loadFilterParams(){
      ...
      if (this.filterQuery.startTime && this.filterQuery.endTime) {
        this.dateRange = [this.filterQuery.startTime, this.filterQuery.endTime]
      }
    },
//点击分析按钮，绘制ECharts
    handleAnalysis(){
    ...
    checkFilterParams () {
      ...
      if (this.dateRange && this.dateRange.length === 2) {
        // this.filterQuery.startTime = this.dateRange[0]
        // this.filterQuery.endTime = this.dateRange[1]
        this.filterQuery.startTime = this.dateRange[0]+' '+'00:00:00'
        this.filterQuery.endTime = this.dateRange[1]+' '+'23:59:59'
      } else {
        this.filterQuery.startTime = ''
        this.filterQuery.endTime = ''
      }
    },
    handleGetProblemRankAnalysis(){
      this.loading = true
      GetProblemRankAnalysis(this.filterQuery).then((res) => {
        ...
        for(let i=0; i<this.problemData.length; i++){
          if(this.problemData[i].rankName){
            this.rankNameList[i] = this.problemData[i].rankName
          }else{
            this.rankNameList[i] = '未填写级别'
          }
          if(this.problemData[i].checkCount){
            this.checkCountList[i] = this.problemData[i].checkCount
          }else{
            this.checkCountList[i] = 0
          }
          if(this.problemData[i].rectiCount){
            this.rectiCountList[i] = this.problemData[i].rectiCount
          }else{
            this.rectiCountList[i] = 0
          }
          if(this.problemData[i].verifyCount){
            this.verifyCountList[i] = this.problemData[i].verifyCount
          }else{
