...
export const getStatisticsInfoByDate = (data) => {
	return request(`/api/getStatisticsInfoByDate?date=${data.date}`, {
		data
	})
}
//删除
...
// 生成监控计划
export const generateMonitorPlan = (data) => {
	return request(`/api/generateMonitorPlan?date=${data.date}&companyCode=${data.companyCode}`, {
		method: 'get'
	})
}
